...
const RecipeComponent = () => {
  ...
  const fetchRecipes = async () => {
    ...
    setError(null);
    try {
      const response = await axios.get(`https://api.edamam.com/search?q=${query}&app_id=160e359c&app_key=67a48b91de75fe5c383a6a4d27850889`);
  ...
  if (selectedRecipe) {
    ...
    return (
      <div>
        ...
        {error && <p>{error}</p>}
        {recipes.map((recipe) => (
          <div key={recipe.recipe.uri} onClick={() => handleRecipeSelect(recipe)}>
